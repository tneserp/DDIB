...
public class ProductReaderConfig {
    ...
    public JpaPagingItemReader<FavoriteProduct> productAlarmReader(){
        ...
        String query = ("select fp from FavoriteProduct fp where (day(fp.product.eventDate) - day(now()) = 1 and hour(fp.product.eventDate) = hour(now())) OR (day(fp.product.eventDate) - day(now()) = 0 and fp.product.eventStartTime - hour(now()) = 1 )");
